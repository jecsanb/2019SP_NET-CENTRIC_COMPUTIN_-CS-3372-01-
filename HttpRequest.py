#!/usr/bin/python3
# classes bellow do not do any data validation


class HttpRequest:
    def __init__(self, string):

    def __init_(self, method, url, version):
        self.REQUEST_LINE = self.RequestLine(method, url, version)
        self.HEADER_LINES = [None]
        self.ENTITY_BODY = None
        self.VALID = True

    def __init__(self):
        self.REQUEST_LINE = None
        self.HEADER_LINES = None
        self.ENTITY_BODY = None
        self.VALID = False

    def getrequestline(self):
        return self.REQUEST_LINE

    def setequestline(self, method, url, version):
        self.REQUEST_LINE = self.RequestLine(method, url, version)

    def getheaderlines(self):
        return self.HEADER_LINES

    def addheaderline(self, name, value):
        self.HEADER_LINES.append(self.HeaderField(name, value))

    def delheaderline(self, name):
        self.HEADER_LINES.

    def getentitybody(self):
        return self.ENTITY_BODY

    def __str__(self):
        return f'{str(self.REQUEST_LINE)}{str(self.HEADER_LINES)}\r\n{self.ENTITY_BODY}'

    class RequestLine:
        def __init__(self, method, url, version):
            self.METHOD = method
            self.URL = url
            self.VERSION = version

        def getmethod(self):
            return self.METHOD

        def geturl(self):
            return self.URL

        def getversion(self):
            return self.VERSION

        def __str__(self):
            return f'{self.METHOD} {self.URL} {self.VERSION}\r\n'

    class EntityBody:
        valid = True

        def __init__(self, data):
            self.DATA = data

        def __str__(self):
            return str(self.DATA)

    class HeaderLines:
        def __init__(self, headerfield):
            self.HEADER_LINES = [headerfield]

        def addfield(self, headerfield):
            self.HEADER_LINES.append(headerfield)

        def count(self):
            return len(self.HEADER_LINES)

        def __str__(self):
            ans = ""
            for line in self.HEADER_LINES:
                ans += str(line)
            return ans

    class HeaderField:
        def __init__(self, name, value):
            self.NAME = name
            self.VALUE = value

        def __str__(self):
            return f'{self.NAME} {self.VALUE}\r\n'


def main():
    rl = RequestLine("GET1", "/index.html", "HTTP/1.1")
    hf = HeaderField("Host:", "hellworld.net")
    hl = HeaderLines(hf)
    hl.HEADER_LINES.append(HeaderField("Connection:", "keep-alive"))
    eb = EntityBody("")
    hr = HttpRequest(rl, hl, eb)
    print(hr, end="")


if __name__ == '__main__':
    main()
